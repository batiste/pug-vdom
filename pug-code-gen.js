...
Compiler.prototype = {
  ...
  visitTag: function(tag, interpolated){
    ...
    function tagName() {
      if (interpolated) return (tag.expr);
      return name;
    }
  ...
  visitAttributes: function(attrs, attributeBlocks){
    ...
    if (attributeBlocks.length) {
      if (attrs.length) {
        var val = this.attrs(attrs, true);
        attributeBlocks.unshift(val);
      }
      if (attributeBlocks.length > 1) {
        this.bufferExpression(this.runtime('attrs', true) + '(' + this.runtime('merge') + '([' + attributeBlocks.join(',') + ']), ' + stringify(this.terse) + ')');
      } else {
        this.bufferExpression(this.runtime('attrs', true) + '(' + attributeBlocks[0] + ', ' + stringify(this.terse) + ')');
      }
    } else if (attrs.length) {
  ...
  attrs: function(attrs){
    ...
    attrs.forEach(function(attr) {
      self.buf.push(`${attr.name}: ${attr.val}`)
    })
  }
};
